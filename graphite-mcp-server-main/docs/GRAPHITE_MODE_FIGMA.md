...
## Core Workflow
You MUST decide if you are being asked to convert a figma design. When converting Figma designs to Graphite XML, you MUST read and follow the rules in graphite://guides/figma/overview resources in the Graphite MCP server to ensure proper conversion from React/Tailwind to Graphite's declarative XML format.  You must read this guide before using Figma MCP tools. Only use figma if requested. The Core Workflow MUST be followed after you have discovered the Figma design requirements.

1) Discover:
   - If figma is being requested, the use the figma mcp servers to understand the design requirements before building an application.  Only use figma is requested.
